 ...
 *
 * <p>Only supports deterministic, alternating-move games.
 *
...
public class MCTS extends AI {
    ...
    protected String analysisReport = "Dupa";
    ...
    public Move selectAction(
        ...
        while (numIterations < maxIts
            ...
            if (!contextEnd.trial().over()) {
                ...
                game.playout(contextEnd, null, -1.0, null, 0, -1, ThreadLocalRandom.current());
            }
        ...
        analysisReport = "[dupa] " + friendlyName + ": best move: " + selectedMove.toString();
        return selectedMove;
    }

    /**
     * Selects child of the given "current" node according to UCB1 equation. This method also
     * implements the "Expansion" phase of MCTS, and creates a new node if the given current node has
     * unexpanded moves.
     *
     * @param current
    ...
    public static Node select(final Node current) {
        if (!current.unexpandedMoves.isEmpty()) {
            ...
            final Move move =
                    current.unexpandedMoves.remove(ThreadLocalRandom.current().nextInt(current.unexpandedMoves.size()));
    ...
    /**
     * Selects the move we wish to play using the "Robust Child" strategy (meaning that we play the
     * move leading to the child of the root node with the highest visit count).
     *
     * @param rootNode
     * @return
     */
    ...
    private static class Node {
        ...
        /**
         * This objects contains the game state for this node (this is why we don't support stochastic
         * games)
         */
